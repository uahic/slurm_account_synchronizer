def check_valid_declared_groups(unix_group_map: dict, config: dict) -> None:
    ...
    def check_group_in_key_exist(grp_entry: dict, key: str, grp_name: str) -> None:
        for unix_grp_name in (grp_entry.get(key) or []):
    ...
    for grp_name, grp_entry in config["declared_groups"].items():
        ...
        check_group_in_key_exist(grp_entry, "groups", grp_name)
        check_group_in_key_exist(grp_entry, "whitelist", grp_name)

        if (
            grp_entry.get("defaultaccount")
